define([
		...
		init=function(id,FX,opt){
			...
			if (opt.showUnit){chshowUnit=opt.chshowUnit;}
		...
		getConfigCOLROW=function(FX){
			//console.log("FX COLROW",FX)
			var ret={AGG:{},COLS:{},ROWS:{},HIDDEN:{},VALS:{}};
			$.each($("#"+id_container+"_AGG >li"),function(e,a){  ret.AGG[a.getAttribute('value')]=true});
			$.each($("#"+id_container+"_ROWS >li"),function(e,a){ret.ROWS[a.getAttribute('value')]=true});
			$.each($("#"+id_container+"_COLS >li"),function(e,a){ret.COLS[a.getAttribute('value')]=true});
			$.each($("#"+id_container+"_HIDDEN >li"),function(e,a){ret.HIDDEN[a.getAttribute('value')]=true});
			$.each($("#"+id_container+"_VALS >li"),function(e,a){ret.VALS[a.getAttribute('value')]=true});
...
//console.log("Ret after extend",ret,ret2)
		...
		showCode=function(){chshowCode= !chshowCode;_onChange();}

		showUnit=function(){chshowUnit= !chshowUnit;_onChange()}

		showFlag=function(){chshowFlag = !chshowFlag;_onChange()}
		...
		display=function()
		{
			...
			for(var i in VALS){$("#"+id_container+"_VALS").append("<li value=\""+VALS[i].value+"\">"+VALS[i].label+"</li>");}
